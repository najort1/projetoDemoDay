...
export default function EditarPerfilCandidato() {
  ...
  const [vulnerabilidadesSelecionadas, setVulnerabilidadesSelecionadas] =
    useState([]);
  ...
  const removePhoto = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setShowModals((prev) => ({ ...prev, showModalSucesso: true }));
        setTituloModal("Foto removida com sucesso !");
        setDescricaoModal("Sua foto de perfil foi removida com sucesso.");

      } else {
        console.error(response.data);
        setShowModals((prev) => ({ ...prev, showModalError: true }));
        setTituloModal("Erro ao remover foto");
        setDescricaoModal(
          "Ocorreu um erro ao remover sua foto de perfil. Tente novamente mais tarde."
        );
    ...
    if (file) {
      ...
      try {
        ...
        if (respostaApi.status === 200) {
          ...
          setShowModals((prev) => ({ ...prev, showModalError: true }));
          setTituloModal("Erro ao atualizar foto");
          setDescricaoModal(
            "Ocorreu um erro ao atualizar sua foto de perfil. Tente novamente mais tarde."
          );
  ...
  const atribuirVulnerabilidades = async () => {
    try {
      const response = await axios.post(
        "http://localhost:8080/vulnerabilidades/atribuir-vulnerabilidade",
        {
          vulnerabilidades: vulnerabilidadesSelecionadas,
        },
        {
          validateStatus: function (status) {
            return status <= 500;
      ...
      if (
        JSON.stringify(response.data).includes("possui essa vulnerabilidade")
      ) {
        setTituloModal("Vulnerabilidades já cadastradas");
        setDescricaoModal("Você já possui essas vulnerabilidades cadastradas.");
        setShowModals((prev) => ({ ...prev, showModalError: true }));
      } else if (response.status === 200) {
        setTituloModal("Dados atualizados com sucesso !");
        setDescricaoModal(
          "Seus dados e vulnerabilidades foram atualizados com sucesso."
        );
        setShowModals((prev) => ({ ...prev, sucesso: true }));
      } else {
        setTituloModal("Erro ao atualizar vulnerabilidades");
        setDescricaoModal(
          "Ocorreu um erro ao atualizar suas vulnerabilidades. Tente novamente mais tarde."
        );
  ...
  const handleEditarInformacoes = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setTituloModal("Erro ao atualizar informações");
        setDescricaoModal(
  ...
  return (
    <>
      ...
      <ModalSessaoExpirada
        ...
        setShowModal={(value) =>
          setShowModals((prev) => ({ ...prev, sessaoExpirada: value }))
        }
      />
      <ModalError
        showModal={showModals.error}
        setShowModal={(value) =>
          setShowModals((prev) => ({ ...prev, error: value }))
        }
        Titulo={tituloModal}
      ...
      <ModalSucesso
        ...
        setShowModal={(value) =>
          setShowModals((prev) => ({ ...prev, sucesso: value }))
        }
        Titulo={tituloModal}
        Descricao={descricaoModal}
      />
      ...
      <ModalConfirmacao
        ...
        setShowModal={(value) =>
          setShowModals((prev) => ({ ...prev, showModalConfirmacao: value }))
        }
        Titulo={tituloModal}
        Descricao={descricaoModal}
      />
          ...
          <div className="caixaInterna bg-white dark:bg-[#6a6a6a]">
            ...
            <form>
              <div>
                ...
                />
                <p className="erro-input-editar-candidato text-red-700 font-bold text-center">
                  {errosInputs.cpf}
                </p>
                ...
                <div className="candidato-selecionar-vulnerabilidade">
                  ...
                  <div className="campo-all-vulnerabilidades h-48 overflow-auto shadow-md border-2 border-blue-800 flex flex-col">
                    {vulnerabilidadesApi.map((vulnerabilidade) => (
                      <div
                        key={vulnerabilidade.nome}
                        className="campo-vulnerabilidade flex flex-row gap-2"
                      >
                        <label htmlFor={vulnerabilidade.nome}>
                          {vulnerabilidade.nome.replace(/_/g, " ")}
                        </label>
                        <input
                          type="checkbox"
                          name="vulnerabilidades"
                          value={vulnerabilidade.nome}
                          checked={vulnerabilidadesSelecionadas.includes(
                            vulnerabilidade.nome
                          )}
                          onChange={(e) => {
                            let updatedVulnerabilidadesSelecionadas;
                            if (e.target.checked) {
                              updatedVulnerabilidadesSelecionadas = [
                                ...vulnerabilidadesSelecionadas,
                                e.target.value,
                              ];
                            } else {
                              updatedVulnerabilidadesSelecionadas =
                                vulnerabilidadesSelecionadas.filter(
                                  (vulnerabilidade) =>
                                    vulnerabilidade !== e.target.value
                                );
                            }
                            setVulnerabilidadesSelecionadas(
                              updatedVulnerabilidadesSelecionadas
                            );
                          }}
                        />
                  ...
                  <div className="vulnerabilidade-customizada flex flex-col">
                    <p
                      className="nao-encontrou-vulnerabilidade font-bold
                      dark:text-white text-center cursor-pointer hover:underline
                      mt-4
                    "
                    >
                      Não encontrou sua vulnerabilidade? Especifique ela nos
                      campos abaixo.
                    </p>

                    <label
                      htmlFor="vulnerabilidade"
